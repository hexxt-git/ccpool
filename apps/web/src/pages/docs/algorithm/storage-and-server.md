...
description: The Storage boundary, runtime portability, and the multi-tenant server with the two-password model.
---

_Part of the [ccpool algorithm docs](/docs/algorithm)._

## Storage — the boundary (and the layer above it)

Two boundaries stack here. The **outer** one is what commands compose (`apps/cli/src/lib/backend.ts`): the daemon writes through an `IngestSink`, views read through a `ViewSource`. On the client both are **always** the HTTP implementations (`HttpIngestSink` / `HttpViewSource`, the latter watermark-cached) — there is no other implementation to pick, and the CLI never opens a database.

The **inner** boundary is `Storage` — used only on the **server**. It's deliberately dumb (rows in, rows out, no business logic), and its one adapter (`LibsqlStorage`, in `@ccpool/storage-libsql`) implements the relational model. Every instance is **scoped to one `groupId`**, bound at construction and injected into every query as a `group_id` column, so one shared database backs every group (see [the server](#the-server--tenancy-and-the-two-password-model)). The interface is unchanged by that — callers see a single ledger:
...
interface Storage {
  ...
  bindAccount(accountId): Promise<void>; // claim an unbound ledger (the "Account binding" section)
  ...
  getChangeToken(): Promise<string>; // 1-row read; the watermark-cache key input
  ...
  getUsageMarkersSince(since): Promise<UsageMarker[]>; // activity markers (see "Attribution")
}
```
...
## The server — tenancy and the two-password model
...
| `GET /v1/view`         | bearer    | the `SharedView`, ETag'd — steady-state polls are bodyless 304s ([the watermark](/docs/algorithm/views#the-watermark--why-a-2s-refresh-is-cheap)) |
